  ...
  void setup() {
    ...
    WiFi.disconnect(true); //disconnecting the device 
    delay(1000); //wait period

    // Connect to the Wi-Fi network created by the first ESP32
    WiFi.softAP(ssid, password); //AP .., Access Point
    ...
    while (WiFi.status() != WL_CONNECTED)
    {
      ...
      if (status != 200){
        Serial.print("Failed connection duo to some serious problems.");
      }
    }
  ...
  void loop() {
    ...
    while (client.available())
    {
      ...
      int time_index = data.indexOf("Time:") + 9;

      String data_slip = data.substring(data_index); //get information out of string
      String time_slip = data.substring(time_index); //same here
